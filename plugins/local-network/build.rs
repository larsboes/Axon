// Every `@objc` handler in `ios/Sources/LocalNetworkPlugin/LocalNetworkPlugin.swift`, and nothing
// else: the generator input for `permissions/`. 1 handler, 1 permission.
...
fn main() {
    tauri_plugin::Builder::new(COMMANDS)
        .ios_path("ios")
        .try_build()
        .expect("failed to build local network plugin permissions");

    // Xcode 27 internalizes @_cdecl exports; same repair as plugins/roomplan/build.rs.
    promote_ios_export("tauri-plugin-local-network", "_init_plugin_local_network");
}
