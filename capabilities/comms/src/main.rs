...
//! controls whether proposals are persisted to the store.

use std::collections::BTreeMap;

use comms::config::{redact_database_url, Config};
use comms::store::{Store, TriageItem};
use comms::{google, media, normalize, rules};
...
fn open_store(cfg: &Config) -> Store {
    Store::open(&cfg.database_url).unwrap_or_else(|e| {
        eprintln!("error: could not open store at {}: {e}", redact_database_url(&cfg.database_url));
        std::process::exit(1);
    })
}
...
fn print_help() {
    ...
    println!("                                  streams, print proposals; persists unless --dry-run");
    ...
    println!("  keep <id>                       mark a feed item 'keeper' (+ export if configured)");
    ...
    println!("\nGmail is READ-ONLY in this build: no modify/trash/delete/label/send calls exist.");
}
...
fn cmd_sweep(args: &[String], cfg: &Config) {
    let limit: usize = arg_after(args, "--limit").and_then(|v| v.parse().ok()).unwrap_or(25);
    ...
    println!("comms sweep — {} inbox threads (READ-ONLY){}\n", stubs.len(), if dry_run { ", dry-run" } else { "" });
    ...
    for stub in &stubs {
        ...
        grouped.entry(stream).or_default().push((from, subject, rationale));
    }
...
fn cmd_ingest(args: &[String], cfg: &Config) {
    ...
    println!("  title   : {}", stored.title.as_deref().unwrap_or("(none)"));
...
fn cmd_feed(args: &[String], cfg: &Config) {
    ...
    let days: i32 = arg_after(args, "--days").and_then(|v| v.parse().ok()).unwrap_or(7);
    ...
    println!("comms feed — {} items (last {days} days){}\n", items.len(),
    ...
    for item in &items {
        ...
        println!("  [{}] {} · {}", item.kind, truncate(title, 70), item.status);
        println!("      {}", item.url);
...
fn cmd_set_status(args: &[String], cfg: &Config, status: &str) {
    let id = match args.get(2) {
        ...
        _ => {
            eprintln!("usage: comms {} <id>", if status == "keeper" { "keep" } else { "dismiss" });
            std::process::exit(1);
        }
    };
...
fn export_keeper(item: &comms::store::FeedItem, dir: &std::path::Path) -> std::io::Result<std::path::PathBuf> {
    std::fs::create_dir_all(dir)?;
    let title = item.title.as_deref().unwrap_or("untitled");
    let day = if item.day.is_empty() { "undated" } else { &item.day };
...
fn slug(s: &str) -> String {
    ...
    if capped.is_empty() { "note".into() } else { capped }
}
