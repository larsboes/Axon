...
- **Work with Axon** — discover and operate running capabilities, feeds, APIs and health
  ...
  runtime through `tools/axon-context` and the `axon capability` CLI verbs, never remembered.
...
It is public-safe by construction — 13 files, no private value in any of them — so it belongs in
Axon beside the other packs rather than in the private overlay.

## Activate
```bash
axon pack deploy claude axon   # → ~/.claude/skills/axon (sync: axon pack sync claude axon)
```
