...
def render(pptx: str | Path, *, out_dir: str | Path | None = None, dpi: int = 110,
           sheets: bool = True) -> dict:
    """Convert a deck to PDF and then to PNGs plus contact sheets.

    Returns `{"pdf": Path, "slides": [Path], "sheets": [Path]}`.
    """
    pptx = Path(pptx)
    if not pptx.exists():
        raise DeckError(
    ...
    return {"pdf": pdf, "slides": slides, "sheets": contact_sheets}
