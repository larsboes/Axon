...
given-new/rhythm) and `references/ai-cadence-tells.md` (the academic handling of the sentence-rhythm
tells — the tells themselves are defined in `human-writing`'s catalog, and word-level filler is
`references/house-style.md` B2/B7). Run reverse-outlining first — it's the highest-signal
...
- **On the em dash, the script is deliberately weaker than the rule.** Pattern 1 matches the
  interruption shape only; `human-writing`'s catalog entry 1 is the standard, and it is stricter —
  any em dash at all, because the rule is not to ship one. Read that entry before dismissing a hit
  as a design choice, and review by hand for the dashes the regex lets through.
