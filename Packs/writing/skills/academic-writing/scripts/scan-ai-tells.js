...
/*
  ...
  The em-dash matcher below is NARROWER than the rule it serves, deliberately. human-writing's
  catalog entry 1 is the standard, and it flags any em dash at all; this regex matches only the
  interruption shape so that a dash inside a table row or an enumeration does not read as a
  finding. A human reviewing against the catalog applies the stricter rule — this is a first-pass
  gate, not the standard.
...
const RHYTHM_PATTERNS = [
  {
    ...
    pattern: /[a-z][^.!?—–\n]{3,80}[—–][^.!?—–\n]{3,120}[—–][^.!?\n]{3,80}[.!?]/g,
    note: 'em-dash interruption (ai-cadence-tells.md #1; human-writing catalog entry 1 is the stricter rule — any em dash) — split into two sentences or use a colon',
  },
  {
