...
A regex cannot judge any of those; they are documented in references/tells-code.md for a human or
...
human pass (see references/tells-code.md). Generic-name detection keys on def / function / func /
fn / fun / sub.
...
def main():
    ...
    if not findings:
        print("  No surface tells flagged. The loudest tells are structural and a regex cannot see\n"
              ...
              "  the rest by hand against references/tells-code.md.\n")
        return
    for rid in rule_ids:
        items = by_rule[rid]
        f0 = items[0]
    ...
    print("  build / type-check / run for the hallucinated calls, read the diff for the rest. See references/tells-code.md.\n")
    sys.exit(by_sev.get("high", 0))
