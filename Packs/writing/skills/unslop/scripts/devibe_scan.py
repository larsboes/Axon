...
def main():
    ...
    if not findings:
        print("  Nothing flagged. Either it is clean or the tells are layout/motion ones a regex"
              "\n  cannot see. Eyeball the hero layout and animations against references/tells-ui.md.\n")
        return
    for rid in rule_ids:
        items = by_rule[rid]
        f0 = items[0]
    ...
    print("  Layout and motion tells need eyes too. See references/tells-ui.md.\n")
    sys.exit(by_sev.get("high", 0))
