...
**Triggers only — never a workflow summary.** A description that sketches the skill's steps
("analyzes the form, maps the fields, then fills and verifies...") becomes a shortcut: the
model follows the summary *instead of reading the body*, skipping every rule that lives there.
Empirically observed in the wild (obra/superpowers documented it from test transcripts). The
description says what the skill is for and when to reach for it; the body owns how.
