...
def parse_frontmatter(path):
    ...
    def field(key):
        ...
        return None
    return field("name"), field("description"), field("when_to_use")
...
def validate(name, description, dirpath=None, extra=None, summary="", when_to_use=None):
    errors = []
    name = name or ""
    description = description or ""

    if when_to_use:
        # Claude Code appends when_to_use to the description in the skill listing and
        # truncates the combined text at 1,536 chars — past that, trailing triggers vanish.
        combined = len(description) + len(when_to_use)
        if combined > 1536:
            errors.append(f"DISCOVERY WARNING: description+when_to_use is {combined} chars; "
                          "the listing truncates at 1536 — put the key use case first or trim.")
...
if __name__ == "__main__":
    ...
    a = p.parse_args()
    if a.file:
        n, d, w = parse_frontmatter(a.file)
        body_errors, summary = check_body(a.file)
        validate(n, d, a.dir or os.path.dirname(os.path.abspath(a.file)), body_errors, summary, w)
    elif a.name:
