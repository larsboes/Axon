...
> Generated: 2026-09-25T18:59:50Z | Source: axon.toml + upstreams.toml + systems.toml + capabilities/ + Packs/
...
| `calendar` | Axon's personal calendar layer: the source of truth for what time means on any given day — availability windows for travel, on‑site work, remote‑work blocks, busy periods, events you're attending, rhythms that materialize those blocks, and (later) day‑planning detail. | — | `8087` | — |
...
| `entities` | Axon's system of record for people, organisations, places and the operator's own context. | — | `8097` | — |
...
| `harness` | Keep the agent system and its repository in shape: operate and change Axon, propose the skills the recorded work actually needs, shrink what loads into every session, and keep every harness in step with the Packs that feed it. | axon, suggest-skills, trim, harness-sync | — |
...
flowchart LR
  ...
  pack_presentations(["presentations"])
  ...
  cap_entities_google_sync["entities-google-sync"] -.->|requires| cap_entities["entities"]
  cap_entities_sync["entities-sync"] -.->|requires| cap_vault["vault"]
  cap_entities_sync["entities-sync"] -.->|requires| cap_entities["entities"]
  cap_feed_sweep["feed-sweep"] -.->|requires| cap_comms["comms"]
  cap_finance_prices["finance-prices"] -.->|requires| cap_finance["finance"]
  cap_punctuality_ingest["punctuality-ingest"] -.->|requires| cap_punctuality["punctuality"]
