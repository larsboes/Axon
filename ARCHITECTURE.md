...
> Generated: 2026-09-07T05:03:00Z | Source: axon.toml + upstreams.toml + systems.toml + capabilities/ + Packs/
...
| `axon-http` | One home for **outbound HTTP**: the blocking client, the user-agent every Axon request carries, and the timeout no caller can forget. |
...
| `deliberation` | Four ways to work a decision: crystallize an unstructured mess into a precise document, convene a council that debates the options, red-team the proposal that wins, and root-cause the failure when one happens anyway. | crystallize, council, red-team, root-cause | — |
| `harness` | Keep the agent harness itself in shape: propose the skills the recorded work actually needs, shrink what loads into every session, and keep every harness in step with the Packs that feed it. | suggest-skills, trim, harness-sync | — |
...
- **adopt**: ibm-plex, mcp-audit, gitleaks, osv-scanner, grype, vaultwarden, bitwarden-cli, r2d2, rusqlite, r2d2_sqlite, bun, maplibre-gl-js, mermaid, vega, vega-lite, vega-embed, openfreemap, wikimedia-api, nominatim-api, open-meteo-api, graphify, asd-ste100-skill, interceptor, bottom, apfel, macmon, typst, yq, vibecoded-design-tells, human-voice, home-assistant, pihole, stevenblack-hosts, tailscale, svelte-ai-tools, sveltejs-mcp, svelte-language-server, pi-coding-agent, debian, deutsche-bahn-data, arrow-rs, xberg, ar5iv, multilingual-e5-base-mlx, bge-reranker-v2-m3-mlx, yahoo-finance-chart, ecb-data-portal, frankfurter-api
- **overlay**: academic-researcher
- **inspiration**: lifeos, iai-personal-memory-engine, local-llm, paperless-ngx, dolphin, ocrs, awesome-selfhosted, colibri, exo, mlx, llama-cpp-rpc, mcpserver-audit, telekom-mcp-security, scout, trek-travel, tripit, betterbahn, besser-bahn, plan-bahn, nvidia-skills, slm-finetuning-talk, pascal-editor, yfinance
...
flowchart LR
  ...
  pack_cv(["cv"])
  pack_cv --> cap_cv["cv"]
  pack_deliberation(["deliberation"])
  pack_harness(["harness"])
  ...
  cap_home_assistant["home-assistant"] -->|image| img_ghcr_io_home_assistant_home_assistant["ghcr.io/home-assistant/home-assistant:stable"]
  cap_pihole["pihole"] -->|image| img_pihole_pihole["pihole/pihole:latest"]
  cap_vaultwarden["vaultwarden"] -->|image| img_vaultwarden_server["vaultwarden/server:alpine"]
  cap_feed_sweep["feed-sweep"] -.->|requires| cap_comms["comms"]
  cap_finance_prices["finance-prices"] -.->|requires| cap_finance["finance"]
